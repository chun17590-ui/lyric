...
// .wrangler/tmp/bundle-yDMP6P/checked-fetch.js
...
// .wrangler/tmp/pages-NNGy7u/functionsWorker-0.7265527536693059.mjs
...
function md5(string) {
  ...
  __name2(AddUnsigned, "AddUnsigned");
  function F(x2, y, z) {
    return x2 & y | ~x2 & z;
  }
  __name(F, "F");
  ...
  __name2(ConvertToWordArray, "ConvertToWordArray");
...
function generateSignature(params, appSecret) {
  const keys = Object.keys(params).filter((k) => k !== "hash" && params[k] !== void 0 && params[k] !== "");
  keys.sort();
  let str = "";
...
async function onRequestPost3(context) {
  try {
    ...
    const params = {
      ...
      notify_url: new URL(request.url).origin + "/api/notify",
      // Auto-detect notify URL
      nonce_str: Math.random().toString(36).substring(2, 15),
      type: "WAP",
      // or 'JSAPI' if inside WeChat, or leave empty for auto-adapt
      wap_url: new URL(request.url).origin,
      // Return url
      modal: null
      // Disable modal if needed or use '1'
    };
...
async function onRequestPost4(context) {
  try {
    ...
    if (!user.is_pro && user.trial_count >= 1e3) {
  ...
  try {
    ...
    if (receivedHash === calculatedHash) {
      if (data.status === "OD") {
        ...
        const open_order_id = data.open_order_id;
        if (env.DB) {
          await env.DB.prepare(
...
// .wrangler/tmp/bundle-yDMP6P/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yDMP6P/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.7265527536693059.js.map
